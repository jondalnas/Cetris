...
unsigned short collision(const piece_t *piece_p, char index, stack_t *stack_p,
                         char x, char y) {
...
int main() {
  ...
  grid_t
      *currPiceGrid_p[4]; // Maybe make pieceToGrid retun array of all rotations
  ...
  stack_t stack = {(int *)calloc(GAME_HEIGHT, sizeof(int)),
                   createNewGrid(GAME_WIDTH, GAME_HEIGHT)};
  ...
  while (!getExitGame()) {
    ...
    if (dy > gravity) {
      if (!collision(currPice_p, rot, &stack, x, y + 1)) {
        ...
        for (char y = 0; y < GAME_HEIGHT; y++) {
          ...
          if ((line & ROW_WIDTH_BITMASK) != ROW_WIDTH_BITMASK)
            continue;
          ...
          *(stack.stack + y) = 0;
          ...
          for (char yy = y - 1; yy >= 0; yy--) {
            *(stack.stack + yy + 1) = *(stack.stack + yy);
            ...
            for (char x = 0; x < GAME_WIDTH; x++) {
              (stack.grid_p->tiles_p + x + (yy + 1) * GAME_WIDTH)->screenObj =
                  (stack.grid_p->tiles_p + x + yy * GAME_WIDTH)->screenObj;
            }
          }
        }
      }
    ...
    drawGridScale(screen_p, stack.grid_p, WIDTH_SCALE,
                  (SCREEN_WIDTH - BORDER_WIDTH) / 2 + 1, 1);
    drawGridScale(screen_p, currPiceGrid_p[rot], WIDTH_SCALE,
                  x * WIDTH_SCALE + (SCREEN_WIDTH - BORDER_WIDTH) / 2 + 1,
                  y + 1);
